...
def generate_primes(num_primes):
    ...
    primes_list = []
    #TODO YOUR CODE HERE
    prime_flags = [True] * (num_primes * 20)
...
def sign_challenge(challenge):
    ...
    # TODO YOUR CODE HERE
    eth_encoded = encode_defunct(text=challenge)
...
def send_signed_msg(proof, random_leaf):
    ...
    contract = w3.eth.contract(address=Web3.to_checksum_address(address),
                               abi=abi)
    tx_unsigned = contract.functions.submit(proof,
    ...
    gas_estimate = w3.eth.estimate_gas({
        'to': tx_unsigned['to'],
        'from': tx_unsigned['from'],
        'data': tx_unsigned['data'],
    })

    tx_unsigned['gas'] = gas_estimate + 10_000


    signed_tx = w3.eth.account.sign_transaction(tx_unsigned, private_key=acct.key)
    sent = w3.eth.send_raw_transaction(signed_tx.raw_transaction)
    tx_hash = sent.hex()
