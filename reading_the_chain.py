...
def is_ordered_block(w3, block_num):
	...
	for tx in txs:
		...
		max_pri = tx.get("maxPriorityFeePerGas")
		max_fee = tx.get("maxFeePerGas")

		if max_pri is not None and max_fee is not None:
			# Type-2 (EIP-1559) tx
			max_pri = int(max_pri)
			max_fee = int(max_fee)
			priority = max(0, min(max_pri, max_fee - base_fee))
		else:
			# Type-0 tx
			priority = max(0, gas_price - base_fee)

		# Check monotonic non-increasing order
...
def get_contract_values(contract, admin_address, owner_address):
	...
	admin_addr_cs = Web3.to_checksum_address(admin_address)
	has_role = contract.functions.hasRole(default_admin_role,
										  admin_addr_cs).call()

	# Call the contract to get the prime owned by "owner_address"
	owner_addr_cs = Web3.to_checksum_address(owner_address)
	prime = contract.functions.getPrimeByOwner(owner_addr_cs).call()
